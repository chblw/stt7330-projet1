...
- La formule $$\frac{1}{N} \sum_{i = 1}^{N} k(\textrm{x}, \textrm{x})$$ est valide si les données sont centrées, c-à-d que $$\sum_{i = 1}^{N} \Phi(\textrm{x}_i) = 0.$$
...
$$ \frac{1}{N} \sum_{i = 1}^{N} \Phi(\textbf{x}_i)\Phi(\textbf{x}_i)^T .$$
...
- ACP non linéaire
- Réduire la dimension davantage
- Noyau = truc utile lorsque produit scalaire requis
- Succès dans détection de visages [@kim2002face]

### Désavantages

- Ajustement d'hyperparamètres
- Non supervisé : ça peut être difficile
- Ne permet pas de reconstruire les données originales
